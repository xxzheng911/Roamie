...
import { homeNearbyOpeningBadgeLabel } from "../src/lib/home-nearby-display.ts";
import { applyFreshHomeNearbyStatusAuthority } from "../src/lib/home-nearby-search.ts";
...
test("permanent/temporary close are hard exclusions; unknown business hours remain eligible", () => {
  assert.equal(
    homeNearbyHardExclusionReason(place({ businessStatus: "CLOSED_PERMANENTLY" })),
    "permanently_closed",
  );
  assert.equal(
    homeNearbyHardExclusionReason(place({ businessStatus: "CLOSED_TEMPORARILY" })),
    "temporarily_closed",
  );
  assert.equal(passesHomeNearbyHardExclusions(place()), true);
  assert.equal(passesHomeNearbyLevel4(place(), "late_night"), false);
  assert.equal(
    passesHomeNearbyHardExclusions(place({ businessStatus: null, openStatus: "unknown" })),
    true,
  );
});

test("fresh provider closure outranks stale operational cache metadata", () => {
  const stale = place({
    businessStatus: "OPERATIONAL",
    openNow: true,
    openStatus: "open",
  ...
  const merged = applyFreshHomeNearbyStatusAuthority(stale, fresh);
  assert.equal(merged.businessStatus, "CLOSED_PERMANENTLY");
  assert.equal(passesHomeNearbyHardExclusions(merged), false);
});

test("Home opening badge requires explicit current-hours evidence", () => {
  assert.equal(
    homeNearbyOpeningBadgeLabel(place({ openNow: true, openStatus: "open" }), "zh-TW"),
    "營業中",
  );
  assert.equal(
    homeNearbyOpeningBadgeLabel(
      place({ openNow: null, openStatus: "unknown", openStatusLabel: "營業中" }),
      "zh-TW",
    ),
    "",
  );
...
test("Home suppresses a thin unknown-hours listing but keeps credible unknown-hours POIs", () => {
  const suspicious = place({
    name: "m",
    openStatus: "unknown",
    openNow: null,
    rating: null,
  ...
  assert.deepEqual(resolveHomeNearbyPoiHealth(suspicious), {
    identityQuality: "thin",
    staleEvidence: true,
    accepted: false,
    dropReason: "suspicious_stale_listing",
  });
  assert.equal(homeNearbyHardExclusionReason(suspicious), "suspicious_stale_listing");
  assert.equal(
    resolveHomeNearbyPoiHealth({ ...suspicious, rating: 4.5, userRatingCount: 120 }).accepted,
    true,
  );
});
...
test("place-focus suppresses stale shortcut and limiter policy remains 20/min concurrent 2", () => {
  ...
  const limiterSource = readFileSync(
    new URL("../src/lib/places-api-guard.ts", import.meta.url),
    "utf8",
  );
  assert.match(recommendationSource, /const shortcutScene = opts\?\.placeDetailNearby\s*\? null/);
  assert.match(
    recommendationSource,
    /const shortcut =\s*params\.placeDetailNearby \|\| params\.searchProfile/,
  );
