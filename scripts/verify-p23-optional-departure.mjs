...
const destination = {
  placeId: "ChIJTaipeiDestination", displayLabel: "台北市", formattedName: "台北市",
  city: "台北市", country: "台灣", address: "台北市", lat: 25.0375, lng: 121.5637,
};
const departure = {
  placeId: "ChIJTaipeiStation", displayLabel: "台北車站", formattedName: "台北車站",
  city: "台北市", country: "台灣", address: "台北市中正區", lat: 25.0478, lng: 121.517,
};
const form = {
  destination, origin: null, days: 2, mood: "", styles: ["城市漫遊", "藝術展覽"],
  startDate: "2026-09-10", endDate: "2026-09-11", departureTime: "", travelers: 1,
  transport: "大眾運輸", budgetMode: "standard",
};
const bundle = { weather: null };

test("destination valid plus blank departure is absent", () => assert.equal(resolvePlanDepartureState("", null), "absent"));
test("whitespace departure is absent", () => assert.equal(resolvePlanDepartureState("   ", null), "absent"));
test("undefined departure is absent", () => assert.equal(resolvePlanDepartureState(undefined, undefined), "absent"));
test("null departure is absent", () => assert.equal(resolvePlanDepartureState(null, null), "absent"));
test("typed but unselected departure is unresolved", () => assert.equal(resolvePlanDepartureState("台北車站", null), "text_unresolved"));
test("selected departure is authoritative", () => assert.equal(resolvePlanDepartureState("台北車站", departure), "selected"));
...
test("Selection handoff accepts absent departure", () => {
  const session = preparePlanTripSession(form, bundle, undefined, { planAiMode: true, localeStyleOptions: form.styles });
  assert.equal(session.tripOrigin, undefined);
...
test("destination remains required", () => {
  assert.throws(() => preparePlanTripSession({ ...form, destination: { ...destination, placeId: "", lat: NaN } }, bundle));
});

console.info(`P23 optional departure: ${passed}/13 passed`);
